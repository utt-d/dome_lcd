...
// LCD表示用のRGBのバッファ
static uint8_t g_lcd_buf[LCD_V_SIZE][LCD_H_SIZE][3]={};
...
void update1HSYNCWithDE(uint8_t g_lcd_h_data[LCD_H_SIZE][3], uint8_t c_data){
	uint16_t c;
	...
	for(c=0;c++;c<CLK_DE_PRE_LOW-CLK_1HSYNC_LOW){
		// 強制的にCLKをHigh, HSYNCをHigh, DEをLowにする
		mcp23s17_write_register(1, MCP23S17_DEFAULT_ADDR, MCP23S17_GPIO, GPIOB, ((c_data & 0x1B) | 0x04) | 0x01);
		// 強制的にCLKをLow, HSYNCをHigh, DEをLowにする
		mcp23s17_write_register(1, MCP23S17_DEFAULT_ADDR, MCP23S17_GPIO, GPIOB, ((c_data & 0x1B) | 0x04) & 0x00);
	...
	for(c=0;c++;c<LCD_H_SIZE){
		// RGBデータを出力する
		mcp23s17_write_register(2, MCP23S17_DEFAULT_ADDR, MCP23S17_GPIO, GPIOA, g_lcd_h_data[c][0]);
		mcp23s17_write_register(2, MCP23S17_DEFAULT_ADDR, MCP23S17_GPIO, GPIOB, g_lcd_h_data[c][1]);
		mcp23s17_write_register(1, MCP23S17_DEFAULT_ADDR, MCP23S17_GPIO, GPIOA, g_lcd_h_data[c][2]);
		// 強制的にCLKをHigh, HSYNCをHigh, DEをHighにする
		mcp23s17_write_register(1, MCP23S17_DEFAULT_ADDR, MCP23S17_GPIO, GPIOB, (c_data | 0x24) | 0x01);
		// 強制的にCLKをLow, HSYNCをHigh, DEをHighにする
		mcp23s17_write_register(1, MCP23S17_DEFAULT_ADDR, MCP23S17_GPIO, GPIOB, (c_data | 0x24) & 0x00);
	...
	for(c=0;c++;c<CLK_1HSYNC_LOW+CLK_1HSYNC_HIGH-CLK_DE_PRE_LOW-CLK_DE_HIGH){
		// 強制的にCLKをHigh, HSYNCをHigh, DEをLowにする
		mcp23s17_write_register(1, MCP23S17_DEFAULT_ADDR, MCP23S17_GPIO, GPIOB, ((c_data & 0x1B) | 0x04) | 0x01);
		// 強制的にCLKをLow, HSYNCをHigh, DEをLowにする
		mcp23s17_write_register(1, MCP23S17_DEFAULT_ADDR, MCP23S17_GPIO, GPIOB, ((c_data & 0x1B) | 0x04) & 0x00);
